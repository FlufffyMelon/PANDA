...
def d_doughnut_delta(l, phi, delta):
    a = -0.25 * pi * (1 - 2 * delta)
    b = (0.25 * pi**2 - 8 / 3) * (0.5 - delta)**2 + 4 * phi * l**2 / (pi * (1 - 2 * delta))
...
def S_doughnut_delta(l, phi, delta):
    ...
    if max(0, phi_min) < phi <= max(0, phi_max):
        ...
        return 0.5 * pi * d**2 + 0.5 * pi**2 * d * (1 - 2 * delta) + pi * (1 - 2 * delta)**2
...
def y_roll_delta(z, l, phi, delta):
    ...
    mask = abs(z) < 0.5 - delta
    z_cut = z * mask
    y = sqrt((1 - 2 * delta)**2 - 4 * z_cut**2) + d

    return y * mask

def rho_roll_delta(z, l, phi, delta):
    return y_roll_delta(z, l, phi, delta) / l
...
def d_perforation_delta(l, phi, delta):
    a = 0.25 * pi * (1 - 2 * delta)
    b = (0.25 * pi**2 - 8 / 3) * (0.5 - delta)**2 + 4 * (1 - phi / (1 - 2 * delta)) * l**2 / pi
...
def S_perforation_delta(l, phi, delta):
    ...
    if max(0, phi_min) < phi <= max(0, phi_max):
        ...
        return 2 * l**2 - 0.5 * pi * d**2 - pi * (1 - 2 * delta)**2 + 0.5 * pi**2 * d * (1 - 2 * delta)
