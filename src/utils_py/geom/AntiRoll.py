...
class AntiRoll(Shape):
    ...
    def generate_point(self) -> np.array:
        Y_min = (self.y(0.5) if (self.theta > np.pi/2) else self.y(0)) * self.borders[2]
        point = np.array([np.random.uniform(-self.borders[0]/2, self.borders[0]/2),
                          (2*np.random.randint(2)-1) * np.random.uniform(Y_min, self.borders[1]/2),
                          np.random.uniform(-self.borders[2]/2, self.borders[2]/2)])
        # point[1] = (2*np.random.randint(2)-1) * np.random.uniform(Y_min, self.borders[1]/2)
