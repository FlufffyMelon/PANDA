...
parser.add_argument("--dependency", type=int, default=-1, help="Added dependency in sbatch script")
...
k = np.sqrt(H * args.phi / (H - 2 * args.offset))
k = np.clip(k, 0, 1)
# assert k < 1, f"This configuration cann't be created! k = {k:.2f}"
decane_insertation_box = Box(
    center=[WIDTH_X / 2, WIDTH_Y / 2, HEIGHT + H / 2],
    borders=[k * WIDTH_X, k * WIDTH_Y, H - 2 * args.offset],
)

water_insertation_anti_box = AntiBox(
    box_center=[WIDTH_X / 2, WIDTH_Y / 2, HEIGHT + H / 2],
    box_borders=[k * WIDTH_X, k * WIDTH_Y, H - 2 * args.offset],
    center=[WIDTH_X / 2, WIDTH_Y / 2, HEIGHT + H / 2],
