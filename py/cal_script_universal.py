...
parser.add_argument(
    "--interface_type", type=str, help="Type of droplet, that will be generated"
)
...
parser.add_argument(
    "--gen_substr", type=str2bool, default=True, help="Generate substrates` gro and itp"
)
...
parser.add_argument(
    ...
    type=float,
    default=1,
    help="Scaling factor of calcite–decane pairwise LJ",
)
parser.add_argument(
...
parser.add_argument(
    "--gpu_id", type=str, default="0", choices=["0", "1", "01"], help="Id of GPU"
)
...
parser.add_argument(
    "--dependency", type=int, default=-1, help="Added dependency in sbatch script"
)

# Remote server params
parser.add_argument(
    "--send_to_server", type=str2bool, default=True, help="Send to remote server flag"
)
...
from src.utils_py.geom.UnionShape import UnionShape  # noqa: E402
...
substr_name = generate_substrate(
    ...
    args.Ly,
    args.Lz,
    freeze_substr=args.freeze_substr,
    build=args.gen_substr,
)
...
module = importlib.import_module(f"src.utils_py.geom.interface_geom")
...
decane_box = Box(
    center=[WIDTH_X * args.phi / 2, WIDTH_Y / 2, HEIGHT + H / 2],
    borders=[WIDTH_X * args.phi, WIDTH_Y, H],
)

water_box = Box(
    center=[WIDTH_X * (1 - args.phi) / 2, WIDTH_Y / 2, HEIGHT + H / 2],
    borders=[WIDTH_X * (1 - args.phi), WIDTH_Y, H],
)

if args.interface_type == "droplet":
    a = np.cbrt(args.phi * WIDTH_X * WIDTH_Y * H)

    decane_insertation_box = Box(
        center=[WIDTH_X / 2, WIDTH_Y / 2, HEIGHT + a / 2 + args.offset],
        borders=[a, a, a],
    )
elif args.interface_type == "doughnut":
    a = np.sqrt(args.phi * WIDTH_X * WIDTH_Y)

    decane_insertation_box = Box(
        center=[WIDTH_X / 2, WIDTH_Y / 2, HEIGHT + H / 2],
        borders=[a, a, H - 2 * args.offset],
    )
elif args.interface_type == "worm":
    a = np.sqrt(args.phi * WIDTH_X * H)

    decane_insertation_box = Box(
        center=[WIDTH_X / 2, WIDTH_Y / 2, HEIGHT + a / 2 + args.offset],
        borders=[a, WIDTH_Y, a],
    )
elif args.interface_type == "roll":
    decane_insertation_box = Box(
        center=[WIDTH_X / 2, WIDTH_Y / 2, HEIGHT + H / 2],
        borders=[args.phi * WIDTH_X, WIDTH_Y, H - 2 * args.offset],
    )

if args.interface_type != "perforation":
    water_insertation_anti_box = AntiBox(
        box_center=decane_insertation_box.center,
        box_borders=decane_insertation_box.borders,
        center=[WIDTH_X / 2, WIDTH_Y / 2, HEIGHT + H / 2],
        borders=[WIDTH_X, WIDTH_Y, H],
    )
else:
    # Special case when generate perforation
    a = np.sqrt((1 - args.phi) * WIDTH_X * WIDTH_Y)

    water_insertation_box = Box(
        center=[WIDTH_X / 2, WIDTH_Y / 2, HEIGHT + H / 2],
        borders=[a, a, H - 2 * args.offset],
    )

    water_insertation_top = Box(
        center=[WIDTH_X / 2, WIDTH_Y / 2, HEIGHT + H - args.offset / 2],
        borders=[WIDTH_X, WIDTH_Y, args.offset],
    )
    water_insertation_bottom = Box(
        center=[WIDTH_X / 2, WIDTH_Y / 2, HEIGHT + args.offset / 2],
        borders=[WIDTH_X, WIDTH_Y, args.offset],
    )
    water_insertation_layer = UnionShape(
        shapeA=water_insertation_top, shapeB=water_insertation_bottom
    )

    water_insertation_region = UnionShape(
        shapeA=water_insertation_box, shapeB=water_insertation_layer
    )

    decane_insertation_anti_box = AntiBox(
        box_center=water_insertation_box.center,
        box_borders=water_insertation_box.borders,
        center=[WIDTH_X / 2, WIDTH_Y / 2, HEIGHT + H / 2],
        borders=[WIDTH_X, WIDTH_Y, H - 2 * args.offset],
    )

if args.interface_type != "perforation":
    insert_shapes = [decane_insertation_box, water_insertation_anti_box]
else:
    insert_shapes = [decane_insertation_anti_box, water_insertation_region]
...
{f"#SBATCH --dependency=afterany:{args.dependency}" if args.dependency != -1 else ""}
...
for name in names:
    if args.send_to_server:
        ...
        os.system(
            f"cp {os.path.join(ff_path, 'itp', str(name) + '.itp')} {os.path.join('~', server_path, str(name) + '.itp')}"
        )
...
if args.send_to_server:
    if args.freeze_substr:
        ...
        os.system(
            f"scp {os.path.join(substr_folder, 'itp', substr_itp_name)} {os.path.join('~', server_path, substr_itp_name)}"
        )
else:
    if args.freeze_substr:
        ...
        os.system(
            f"cp {os.path.join(substr_folder, 'itp', substr_itp_name)} {os.path.join('~/', server_path, substr_itp_name)}"
        )

if args.send_to_server:
    os.system(
        f"scp {os.path.join(exp_path, str(ff_type) + '.top')} {server_name}:{server_path}"
    )
else:
    os.system(
        f"cp {os.path.join(exp_path, str(ff_type) + '.top')} {os.path.join('~/', server_path, str(ff_type) + '.top')}"
    )
...
for file in files:
    if args.send_to_server:
        ...
        os.system(
            f"cp {os.path.join(exp_path, file)} {os.path.join('~/', server_path, file)}"
        )
...
if args.send_to_server:
    ...
    os.system(
        f"cp {os.path.join(exp_path, filename + '.gro')} {os.path.join('~/', server_path, f'{filename}_init.gro')}"
    )

if args.send_to_server:
    os.system(
        f"scp {os.path.join(exp_path, 'config.json')} {server_name}:{server_path}/config.json"
    )
else:
    os.system(
        f"cp {os.path.join(exp_path, 'config.json')} {os.path.join('~/', server_path, 'config.json')}"
    )
